...
interaction_terms = poly_features.fit_transform(df_cleaned[['age', 'english.grade', 'math.grade', 'sciences.grade', 'language.grade']])
interaction_df = pd.DataFrame(interaction_terms, columns=poly_features.get_feature_names_out(['age', 'english.grade', 'math.grade', 'sciences.grade', 'language.grade']))
df_cleaned = pd.concat([df_cleaned, interaction_df], axis=1)

encoder = OneHotEncoder(drop='first', sparse_output=False)
encoded_features = encoder.fit_transform(df_cleaned[['gender', 'nationality', 'city']])
encoded_df = pd.DataFrame(encoded_features, columns=encoder.get_feature_names_out(['gender', 'nationality', 'city']))
df_cleaned = pd.concat([df_cleaned, encoded_df], axis=1)
df_cleaned = df_cleaned.drop(['gender', 'nationality', 'city'], axis=1)
...
if target in df_cleaned.columns:
    ...
    for name, model in models.items():
        ...
        if hasattr(model, 'predict'):
            ...
            partial_dependence.plot_partial_dependence(model, X_train_scaled, features.columns, ax=ax)
